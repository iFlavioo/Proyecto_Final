...
class TransferenciaControllerIT {
    ...
    void testEliminar() throws Exception {
        Transferencia guardado = transferenciaRepository.save(new Transferencia(null, 1L, 1L, 2L, 5, "PENDIENTE", java.time.LocalDate.now()));
    ...
    void testActualizar() throws Exception {
        Transferencia guardado = transferenciaRepository.save(new Transferencia(null, 1L, 1L, 2L, 5, "PENDIENTE", java.time.LocalDate.now()));
        TransferenciaDTO dto = new TransferenciaDTO(null, 1L, 1L, 2L, 10, "APROBADA", java.time.LocalDate.now());
        mockMvc.perform(put("/api/v1/transferencias/"+guardado.getId()).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto)))
    ...
    void testObtenerPorIdNoExistente() throws Exception {
        mockMvc.perform(get("/api/v1/transferencias/9999")).andExpect(status().isNotFound());
    }
}
