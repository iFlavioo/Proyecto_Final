...
class ResenaControllerIT {
    ...
    void testEliminar() throws Exception {
        Resena guardado = resenaRepository.save(new Resena(null, 1L, 1L, "Excelente libro, muy recomendado", 4, java.time.LocalDate.now()));
    ...
    void testActualizar() throws Exception {
        Resena guardado = resenaRepository.save(new Resena(null, 1L, 1L, "Excelente libro, muy recomendado", 4, java.time.LocalDate.now()));
        ResenaDTO dto = new ResenaDTO(null, 1L, 1L, "Actualizo mi opinion: gran libro", 5, java.time.LocalDate.now());
        mockMvc.perform(put("/api/v1/resenas/"+guardado.getId()).contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(dto)))
    ...
    void testObtenerPorIdNoExistente() throws Exception {
        mockMvc.perform(get("/api/v1/resenas/9999")).andExpect(status().isNotFound());
    }
}
